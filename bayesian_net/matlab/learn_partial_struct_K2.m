function dag = learn_partial_struct_K2(data, ns, order, varargin)
% LEARN_PARTIAL_STRUCT_K2, modified version of LEARN_STRUCT_K2 (see below)
% allowing for specification of an initial DAG and limitation of learining
% to nodes in LEARNNODES. Also the set of possible partens can be
% specified. Additional arguments:
%
% initdag    - initial dag specification
% learnnodes - learn only structure for these nodes
% potparents - use only these nodes as potential parents
%
  ...
  if isstr(args{1})
    for i=1:2:nargs
      switch args{i},
       ...
       case 'initdag',    dag=args{i+1}; if (size(dag,1)~=n || size(dag,2)~=n), dag=zeros(n,n); end
       case 'learnnodes', learnnodes=args{i+1};
       case 'potparents', potparents=args{i+1};
      end
    end
  else
    max_fan_in = args{1};
...
for i=learnnodes
  ...
  while (~done && (length(ps) < max_fan_in))
    pps = mysetdiff(order(potparents(potparents<i)), ps); % potential parents
    nps = length(pps);
    pscore = zeros(1, nps);
    for pi=1:nps
      p = pps(pi);
